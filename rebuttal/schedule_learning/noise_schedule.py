...
class LearnablePolynomialNoise(Noise):
  """Learnable monotone noise schedule for SCDD."""

  def __init__(self, sigma_min: float = 0.0, sigma_max: float = 6.91):
    super().__init__()
    self.sigma_min = sigma_min
    self.sigma_max = sigma_max
    self.psi_1 = nn.Parameter(torch.tensor(1.0))
    self.psi_2 = nn.Parameter(torch.tensor(0.0))
    self.psi_3 = nn.Parameter(torch.tensor(1.0))

  def _f(self, t: torch.Tensor) -> torch.Tensor:
    p1, p2, p3 = self.psi_1, self.psi_2, self.psi_3
    return (
      (p1**2 / 5) * t**5
      + (p1 * p2 / 2) * t**4
      + ((p2**2 + 2 * p1 * p3) / 3) * t**3
  ...
  def _f_prime(self, t: torch.Tensor) -> torch.Tensor:
    return (self.psi_1 * t**2 + self.psi_2 * t + self.psi_3) ** 2

  def _normalizer(self, t: torch.Tensor) -> torch.Tensor:
    ones = torch.ones(1, device=t.device, dtype=t.dtype)
    return self._f(ones) + 1e-8

  def total_noise(self, t: torch.Tensor) -> torch.Tensor:
    normalized = self._f(t) / self._normalizer(t)
    return self.sigma_min + (self.sigma_max - self.sigma_min) * normalized
  ...
  def schedule(self, t: torch.Tensor) -> torch.Tensor:
    return torch.exp(-self.total_noise(t))


def get_sigma_max_gamma(gamma_floor: float = 1e-3) -> float:
  return -math.log(gamma_floor)


def get_sigma_max_rho(
  sigma_max_gamma: float,
  p_u: float = 0.1,
  n_grid: int = 2000,
  tol: float = 1e-4,
) -> float:
  u = torch.linspace(0, 1, n_grid)

  def peak(s_rho):
    gamma_u = torch.exp(-sigma_max_gamma * u)
    rho_u = torch.exp(-s_rho * u)
    return ((1 - rho_u) * gamma_u).max().item()
